...
def csvdir_bundle(
    ...
    show_progress,
    output_dir,
    bigfile=None,  # 新增参数，传入大表文件路径
):
    """
    Build a zipline data bundle from a single big csv/parquet file.
    """
    if not bigfile:
        bigfile = environ.get("CSVFILE")
        if not bigfile:
            raise ValueError("CSVFILE environment variable is not set")

    df = pd.read_parquet(bigfile) if bigfile.endswith(".parquet") else pd.read_csv(bigfile, parse_dates=["date"])
    ...
    daily_bar_writer.write(
        _pricing_iter(bigfile, symbols, metadata, divs_splits, show_progress),
        show_progress=show_progress,
    )

    metadata["exchange"] = "CSVDIR"
    metadata = metadata[metadata["symbol"].notnull() & (metadata["symbol"] != "")]
...
def _pricing_iter(bigfile, symbols, metadata, divs_splits, show_progress):
    df = pd.read_parquet(bigfile) if bigfile.endswith(".parquet") else pd.read_csv(bigfile, parse_dates=["date"])
    grouped = df.groupby("symbol")
    for sid, symbol in enumerate(tqdm(symbols, desc="Ingesting symbols", disable=not show_progress)):
        try:  
            dfr = grouped.get_group(symbol).set_index("date").sort_index()
            ...
            if "split_ratio" in dfr.columns:
                tmp = 1.0 / dfr[dfr["split_ratio"] != 1.0]["split_ratio"]
